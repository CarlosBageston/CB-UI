...
const CBCard: React.FC<CBCardProps> = ({
  ...
  const subtitleColor =
    theme?.colors?.subtitle || "text-gray-500 dark:text-gray-400";
        ...
        >
          <div className="flex justify-between w-full items-center">
            <div className="flex flex-col text-center w-full">
              {title && (
                <IonCardTitle className={`text-lg font-semibold ${textColor}`}>
                  {title}
                </IonCardTitle>
              )}
              {subtitle && (
                <IonCardSubtitle className={`${subtitleColor} text-sm`}>
                  {subtitle}
                </IonCardSubtitle>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {onClickEdit && onClickEdit && (
                <MdEdit
                  ...
                  className={clsx(
                    ...
                    theme?.colors?.edit || "text-(--cb-color-secondary-shade)",
                  )}
                />
              )}
              {onClickDelete && onClickDelete && (
                <MdDeleteForever
                  ...
                  className={clsx(
                    "w-10 h-10 p-2 rounded-md hover:scale-110 transition bg-gray-50 backdrop-blur-sm",
                    theme?.colors?.delete || "text-(--cb-color-danger)",
                  )}
                />
              )}
              {collapsible && (
